...
class MemberService {
    ...
    public async signup(input: MemberInput): Promise<Member> {
        ...
        try {
            const result = await this.memberModel.create(input); // memberSchema model is Class
    ...
    public async getTopUsers(): Promise<Member[]> {
        const result = await this.memberModel.find({
            ...
            memberPoints: { $gte: 1}}).sort({memberPoints: -1}).limit(4).exec();
        if(!result) throw new Errors(HttpCode.NOT_FOUND, Message.NO_DATA_FOUND);
...
export default MemberService; //classni pascal harflar bilan yozamiz


//promise is used only with async